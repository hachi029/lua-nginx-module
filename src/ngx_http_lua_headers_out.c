 ...
 * 
 * 用来构建初始化哈希表 lmcf->builtin_headers_out
 * 
 */
ngx_int_t
ngx_http_lua_init_builtin_headers_out(ngx_conf_t *cf,
    ngx_http_lua_main_conf_t *lmcf)
{
    ngx_array_t                   headers;      //临时动态数组，元素类型为 ngx_hash_key_t
    ngx_hash_key_t               *hk;
    ngx_hash_init_t               hash;
    ngx_http_lua_set_header_t    *handlers = ngx_http_lua_set_handlers;
    ngx_uint_t                    count;

    //ngx_http_lua_set_handlers 数组元素个数
    ...
    //将headers数组中的所有元素的key,  元素地址放入headers数组
