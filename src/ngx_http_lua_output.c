...
ngx_http_lua_ngx_flush(lua_State *L)
{
    ...
    //wait: switches to synchronous mode. In synchronous mode, 
    //the function will not return until all output data has been written into the system send buffer 
    //or until the send_timeout setting has expired
    if (n == 1 && r == r->main) {
    ...
    // 获取一个ngx_chain_t结构体cl，其buf的flush被置位
